...
import Util.{getMovieDataDF, spark}
...
object explore_movies {
  ...
  def main(args: Array[String]) {
    ...
    val raw_title = getMovieDataDF().select("name")
    raw_title.show()

    raw_title.createOrReplaceTempView("titles")
    spark.udf.register("processRegex", processRegex _)
    val processed_titles = spark.sql("select processRegex(name) from titles")
